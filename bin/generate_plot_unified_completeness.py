...
Edits by Tomas Bruna
   - Complete genes are not split between Single and Duplicated
   - Information about busco set is added into the figure
"""
...
RCODE = '######################################\n'\
        ...
        '\n'\
        '# Colors\n'\
        'my_colors <- c("#56B4E9", "#F0E442", "#F04442")\n'\
        '# Bar height ratio\n'\
        'my_bar_height <- 0.75\n'\
        '\n'\
        '# Legend\n'\
        ...
        '  scale_fill_manual(values = my_colors,labels =c(" Complete (C)  ",\n'\
        '                                                 " Fragmented (F)  ",\n'\
        '                                                 " Missing (M)")) +   \n'\
        '  ggtitle(my_title, subtitle = "Lineage dataset: %s7 (built from %s8 species)") + \n'\
        '  xlab("") + \n'\
        '  ylab("\\n%BUSCOs") + \n'\
        '\n'\
        '  theme(plot.title = element_text(family=my_family, hjust=0.5, colour = "black", size = rel(2.2)*my_size_ratio, face = ' \
        '"bold.italic")) + \n'\
        '  theme(plot.subtitle = element_text(family=my_family, hjust=0.5, colour = "black", size = rel(1.6)*my_size_ratio)) + \n'\
        ...
        '    figure <- figure + \n' \
        '    annotate("text", label=paste("C:", detailed_values[1], '\
        '", F:", detailed_values[2], ", M:", detailed_values[3], ", n:", total_buscos, ' \
        'sep=""), \n' \
...
def _write_r_code(data):
    ...
    r_file.write(RCODE
                 ...
                 .replace('%s5', '(1)')
                 .replace('%s6', data['speciesName'])
                 .replace('%s7', data['lineage'])
                 .replace('%s8', data['numSpecies']))
...
def _load_data():
    ...
    :return:
    """
    data = {'species': [], 'values': [], 'percentages': [], 'species_tmp': [], 'lineage': [], 'numSpecies': [], 'speciesName': []}
    datasets = set([])
        ...
        try:
            ...
            for line in content:
                if 'The lineage dataset is' in line:
                    match = re.search('The lineage dataset is: (.*) \(', line)
                    data['lineage'] = match.group(1)
                    match = re.search('number of species: (.*),', line)
                    data['numSpecies'] = match.group(1)
                if 'Summarized benchmarking in BUSCO ' in line:
                    match = re.search('BUSCO notation for file (.*)', line)
                    data['speciesName'] = match.group(1).replace("_", " ")
            ...
            data['species_tmp'] += [".".join(f.split('/')[-1].split('.')[3:-1]+[f.split('/')[-1].split('.')[2]])]*3
            comp += dupl
            data['values'] += [comp, frag, miss]
            total = comp + frag + miss
            comp_pc = round(comp/float(total)*100, 1)
            frag_pc = round(frag/float(total)*100, 1)
            miss_pc = round(100 - comp_pc - frag_pc, 1)
            data['percentages'] += [comp_pc, frag_pc, miss_pc]
